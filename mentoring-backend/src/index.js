...
// Middlewares
// app.use(helmet()); // TEMP disabled for debug

// SAFE DEBUG VERSION - allows all origins
app.use(cors({
  origin: "*",
...
app.get('/api/mentors', async (req, res) => {
  try {
    ...
    // --- SEARCH FILTER (name, title, domains, topics) ---
    if (q && q.trim() !== "") {
      const searchText = q.trim();
      filter.$or = [
        { name: { $regex: searchText, $options: "i" } },
        { title: { $regex: searchText, $options: "i" } },
        { domains: { $regex: searchText, $options: "i" } },
        { topics: { $regex: searchText, $options: "i" } }
      ];
    }
    ...
    console.log('GET /api/mentors - found', mentors.length, 'mentors');
